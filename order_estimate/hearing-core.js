...
(function () {
  ...
  var DEFAULT_KB = {
    ...
    sizes: [
      ...
      { id: 'a5slim', name: 'A5スリム', 穴: '6穴', 寸: '110×210mm', hint: 'A5と同じ高さのまま幅を38mm細く。大判の書きやすさは残しつつ、かばんに差しやすい。' }
    ],
    rings: {
    ...
    // 2026-07-15 Phase4-③: 用途→サイズの市場調査（メーカー公式・専門店ガイド）に基づく提案ヒント。
    // 正本はhearing-kb.jsonのsizeAdvice（管理画面から編集可）。根拠: デジタル部\サイト管理\AI相談窓口_Phase4_市場調査_用途別サイズ.md
    sizeAdvice: 'サイズ選びの目安（手帳メーカー・専門店ガイドの市場傾向 2026-07調査）: バイブルは業界の標準サイズでリフィルの種類が最も豊富。仕事・プライベート両用の万能型で、迷ったらまずバイブルをおすすめ。A5は書き込み量重視でA4書類を半分に折って挟めるためビジネスの書類集約に強いが、かさばるのでデスク・自宅常置向き。ミニ6はシャツや上着のポケットに入る常時携帯サイズで、TO DO・打合せメモなどメモ中心の使い方やスマホでスケジュール管理する方に好相性。Micro5はさらに小さい超携帯メモ用。用途と持ち運び頻度を聞いたうえで、記入量と携帯性のバランスで提案する。',

    models: [
      // 2026-09-13: 構造カードは Standard に一本化。オプションを一切付けない状態を Simplist と呼ぶ
      { id: 'simplist', name: 'Standard', desc: 'リングをサポーターで包み込む軽量スタイル。ポケット等のオプションを一切付けない状態を Simplist と呼ぶ。', for: '軽さ・シンプルさ・革そのものが好きな方' },
      // 2026-09-12: 見積もりシミュレーターの構造名に合わせて追加（旧名 Simplist＋クラウゼ＋Pad）
      { id: 'simplist_klpad', name: 'N-Classic', desc: 'pad加工で内側にも革を張った構造。クラウゼリングも通常リングも選べる。', for: 'しっかりした作りで、リングの種類も選びたい方' },
      { id: 'npad_nf',  name: 'N-Pad（フラップ無）', desc: '内張り付きでしっかりした作り。かっちり手帳らしい佇まい。', for: 'きちんと感・耐久性を重視する方' },
      // 2026-09-12: N-Pad flap は N-Pad（フラップ無）にフラップとペンホルダーを付けた構成（見積もりと同じ考え方）
      { id: 'npad',     name: 'N-Pad（フラップ有）', desc: 'N-Pad（フラップ無）に、フラップ（かぶせ）とペンホルダーを付けた構成。中身をしっかり保護できる。', for: '持ち歩きが多く中身を守りたい方（フラップ+3,000円・ペンホルダー+800円／税抜・外すことも可）' },
  ...
  function buildSystemPrompt(KB, leathers, stockMap, stitches) {
    ...
    if (stitches && stitches.length) {
      ...
      stitchSection = '■ステッチ糸（ビニモMBT・全' + stitches.length + '色）\n'
        ...
        + '※在庫があるのは次の番号だけ: ' + stitches.filter(function (x) { return x.inStock; })
            .map(function (x) { return x.label; }).join(' , ') + '。ここに無い番号は全てお取り寄せ。\n'
        + '※在庫の有無を答えるときは、必ずこの番号リストと照合する。ここに無い番号を「在庫あり」と言わない。\n'
    ...
    return 'あなたは「' + KB.shop + '」のオーダー相談AI「幕僚（ばくりょう）」です。革職人 中司祐樹の名代として、お客様からレザーアイテムのフルオーダー（システム手帳・財布・キーケース・名刺入れ・ポーチ/ペンケース・コインケース 他）のご要望を丁寧にヒアリングし、プロの視点で提案します。\n\n' +
      ...
      '3. アイテム固有の基本仕様（形・容量・機能・リングサイズ・カード枚数 など）\n' +
      '4. 細部仕様・オプションパーツ（下記のうち該当するものを具体的に確認する。位置・大きさ・数まで詰める）:\n' +
      '   - ポケット: 数／位置（前面・背面・内側・見返し）／種類（カードサイズ・フリーポケット・フラップ付・マチ付）\n' +
      '   - ペンホルダー: 有無／位置（背表紙・内側・サイド）／対応ペンの太さ（差し込み式かループか。太軸ペンなら径を確認）\n' +
      '   - Dカン・ナスカン・金具: 有無／位置／サイズ（ストラップ・チャーム・キーホルダー取付用か）\n' +
      '   - 仕切り・マチ・開閉方法（ファスナー／スナップ／かぶせ／ゴムバンド 等）\n' +
      '   - コバ（革の切り口）の色・仕上げ、その他ステッチの入れ方の希望\n' +
      '5. 革の色（希望があれば革の種類・質感も）\n' +
      '6. ステッチ色\n' +
      '7. 金具・リングの色（該当するアイテムのみ）\n' +
      '8. 名入れ（有無・入れる文字・位置）\n' +
      '9. ご予算\n' +
      '10. 納期・お渡し希望時期（贈り物なら渡す日）\n' +
      '11. 参考イメージの有無（写真やイラストがあれば、チャットの📷添付ボタンから画像を添付でき、ご相談内容と一緒に工房へ届くと案内する）\n' +
      '質問は1回に1つずつ。順番はこの通りでなくてよく、お客様の話の流れに合わせて柔軟に聞く。お客様がすでに話した内容は聞き直さない。「おまかせ」「こだわらない」と言われた項目は深追いせず「おまかせ」として扱う。\n' +
      '細部仕様（項目4）は、そのアイテムに関係するものだけを、用途に合わせて自然に確認する（無関係な項目まで機械的に質問攻めにしない）。位置や大きさは「どのあたりに」「どのくらいの大きさか」と具体的にイメージを引き出す。\n' +
      'お客様が「こだわらない・実用的でいい・おまかせ」と言った場合でも、そのアイテムの主要な細部オプション（例：手帳ならペンホルダーの有無・Dカン/金具の有無・ポケットの構成）は、決めつけずに一度こちらから「〜は付けますか？」と選択肢として提示して意向を確認する。そのうえで改めて「おまかせ」と言われた項目だけ「おまかせ」として扱う。細部を一度も尋ねないまま発注票に進まない。\n' +
      '発注票をまとめる前に、項目1〜11をすべて見直す。とくに細部仕様のうち【ポケットの構成・ペンホルダーの有無と位置・Dカン/金具の有無】の3点は、一度も触れていなければ発注票を出す前に必ずまとめて確認する（お客様の意向が「付けない／おまかせ」でも、確認した事実を残す）。未確認の項目が残っていれば先にそれを確認する。ひと通り揃ったら最後に発注票を「【発注票】」から箇条書きでまとめ、細部仕様も1項目ずつ記載し、おまかせ・未確認の項目はその旨も明記する。\n\n' +
      '【工房の知識ベース】\n' +
      '■システム手帳サイズ: ' + KB.sizes.map(function (s) { return s.name + '(' + s.寸 + ') ' + s.hint; }).join(' / ') + '\n' +
      (KB.sizeAdvice ? '■' + KB.sizeAdvice + '\n' : '') +
