...
(function () {
  'use strict';
  const SIMULATOR_VERSION = '2026.09.05.5';
  ...
  function commitPoint(index, area, hoursValue) {
    ...
    // Prioritize the edited point; keep the curve monotone by moving only conflicting neighbors.
    const ps = candidate.curves[state.brand];
    for (let i = index - 1; i >= 0; i--) ps[i].hours = Math.min(ps[i].hours, hoursValue);
    for (let i = index + 1; i < ps.length; i++) ps[i].hours = Math.max(ps[i].hours, hoursValue);
  ...
  function render() {
    ...
    const circles = points.map((p, i) => {
      ...
      c.addEventListener('keydown', e => {
        ...
        const value = Math.max(0, ps[i].hours + (e.key === 'ArrowUp' ? .05 : -.05));
    ...
    svg.onpointermove = e => {
      ...
      const h = Math.max(0, Math.round((state.drag.startHours + (state.drag.startY - pt.y) / ph * ymax) * 20) / 20);
      commitPoint(i, ps[i].area, h); draw();
      state.data.curves[state.brand].forEach((point, index) => { const input = host.querySelector(`[data-hours="${index}"]`); if (input) input.value = point.hours; });
    ...
    host.append(el('p', '金額 = 基本料金 ＋ 曲線の工数 × 時給 ＋ 構造の固定加算 ＋ 革材料費 ＋ オプション。動かした点を優先し、面積と工数の大小関係が逆転する隣接点の工数も合わせます。'));
