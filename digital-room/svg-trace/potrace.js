/*
 * N's factory イラストSVG変換 用の配置版（2026-09-11）
 * 原本: https://github.com/kilobtye/potrace （GPL）
 * 変更点: 末尾に loadBitmapData() と getCurves() を追加しただけで、
 *         トレース処理そのものには手を入れていない。
 */
...
var Potrace = (function() {
  ...
  // ── N's factory 追加（2026-09-11）─────────────────────────
  // 前処理済みの白黒データを直接読み込む（data: 1=図柄, 0=背景, 長さ w*h）
  function loadBitmapData(w, h, data) {
    clear();
    bm = new Bitmap(w, h);
    for (var i = 0; i < bm.size; i++) {
      bm.data[i] = data[i] ? 1 : 0;
    }
    info.isReady = true;
  }

  // トレース結果を座標のまま返す（SVG組み立ては呼び出し側で行う）
  // 戻り値: [{ n, tag:["CURVE"|"CORNER",...], c:[{x,y},...] }]
  function getCurves() {
    var out = [], i, j, cv;
    for (i = 0; i < pathlist.length; i++) {
      ...
      var pts = [];
      for (j = 0; j < cv.n * 3; j++) {
        pts.push(cv.c[j] ? { x: cv.c[j].x, y: cv.c[j].y } : null);
      }
      out.push({ n: cv.n, tag: cv.tag.slice(), c: pts });
    }
    return out;
  }
  // ────────────────────────────────────────────────────────
