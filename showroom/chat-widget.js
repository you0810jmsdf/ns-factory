  ...
  function mount(container, opts) {
    ...
    function openChatInner(staffId, productContext) {
      ...
      if (currentProductContext && currentProductContext.name) {
        ...
        // 革色・ステッチ色以外の要望や、メール以外の連絡先の書き方が分からず
        // 埋もれてしまわないようにするための一言。
        if ($('#oc-purchase-method')) {
          appendStaffMsg('サイズの変更など、他にもご要望があればこのメッセージ欄に自由にご記入ください。ご連絡先がメール以外（LINE・Instagramなど）の場合は、ご利用のSNS名とIDをこちらにご記入いただけると助かります。');
        }
      }
    ...
    var NSF_MATERIAL_GUIDE_ADDENDUM = '\n\n【自作派（レザークラフトDIY）サポート知識】' +
      ...
      // 2026-09-14: 馬蹄型の ¥2,800 版（STORES）と材料調達ガイドの案内を削除（N's pattern 版は販売しない・事業主決定）。材料の知識は残す。
      '■馬蹄型コインケースの必要材料: 表革=タンニン鞣しヌメ革A4×1枚（サドルレザーが初心者向け）／バネホック1組／ロウ引き糸0.8〜1.0mm 約1m／レザー用丸針2本／菱目打ち2・4本目（ピッチ3mm）／革包丁またはカッター／ゴム板／コバ仕上げ剤（トコノール等）。' +
      '■購入先の使い分け: 少量・送料重視なら楽天市場のはぎれ・カット革、産地や鞣しの明確さ重視なら専門店（誠和・協進エル・レザーマニア）。' +
      '■自作の相談（材料・道具・作り方・型紙）を受けたら上記の知識で答える。' +
    ...
    function showOrderFollowupNotice() {
      ...
      var howTo = hasContactField
        ? 'お手数ですが、この画面上部の「お名前」「連絡先（メール／SNS）」欄をご記入のうえ、もう一度「この内容で作家に送信」ボタンを押していただけますでしょうか。'
      ...
      appendStaffMsg(
        ...
        + howTo + '\n'
        + 'ご連絡先は、メールアドレスのほか、LINEやInstagramのIDでも構いません。\n\n'
