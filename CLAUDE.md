...
## 2026-08-26 — SNS動画メーカー: フィルムの流れる速さセレクトを追加（動きの強さ連動を廃止）

- 対象: `digital-room/sns-video-maker/index.html`
- 事業主が実機で操作確認のうえ「スピードを調整したい」。専用セレクト `filmSpeed`（ゆっくり3.5／標準2.2／速め1.5／速い1.0 秒/コマ）を追加し、`drawFilmStripScene` の速度算出を `pitch / secPerFrame` に一本化。
- **「動きの強さ」との連動は廃止**。2系統で速度が変わると狙った速さに調整できないため。前日の実装メモにある係数対応表（1/1.6/2.1/2.7）は無効。
- `filmSpeedField` はフィルムシーンON時のみ表示（`filmStripMode` の change でトグル。初期はHTML属性 hidden）。
- 検証（ブラウザ実測）: OFF時非表示→ON時表示→OFF復帰で非表示／同t・3速度で全画面ハッシュがすべて異なる／動きの強さ standard⇔max で描画不変（非連動）／速い設定でも1周期後のストリップが3周期まで完全一致。
  - ⚠ 検証の注意: セリフ入りだと「1周期後の完全一致」は**成立しない**のが正しい（字幕はストリップではなくタイムラインに追従するため）。ループ検証は narration/caption を空にして行うこと。
